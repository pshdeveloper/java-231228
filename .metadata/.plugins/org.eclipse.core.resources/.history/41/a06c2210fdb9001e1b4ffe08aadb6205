...
// LinkedList: 내부적으로 이중 연결된 리스트를 사용하여 관리
// : 내부적으로 이중 연결 리스트를 사용
...
public class B_List {
	public static void main(String[] args) {
		...
		// - indexOf(Object o): 지정된 요소가 처음으로 나타는 위치의 인덱스를 반환
		// - contains(Object o): 리스트가 특정 요소를 포함하느지 여부를 반환
		// - set(int index, E e): 지정된 위치에 주어진 요소를 저장, 이전에 있던 요소를 반환
		
		//! 1. ArratList 사용 예시
		// : new 생성자를 사용하여 ArrayList 객체를 사용
		...
		// 배열의 해당 요소가 포함되어 잇는지 확인
		String isFruit = "Orange";
		String isFruit2 = "Strawberry";
		
		System.out.println(linkedList.contains(isFruit));
