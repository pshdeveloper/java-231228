...
// : , 상속하고자 하는 클래스의 이름을 키워드 'extends'와 함꼐 작성
...
// : 자식클래스는 부모 클래스의 모든 멤버를 상속받기 때문에
// : , 자식 클래스의 멤버 개수는 부모클래스보다 항상 같거나 많음
// 		>> 상속에 사용되는 키워드가  extends 인 이유
...
// 상속 예시
// : Tv 클래스로부터 상속받고 기능을 추가하여 SmartTv 클래스를 작성
class Tv {
	// 전원 상태(on/off)
	boolean power;
	// 채널
...
class SmartTv extends Tv {
	...
	void displayOtt(String platform) {
		if (ott) { // OTT 의 상태가 on(true)일 떄만 platform 을 보여줌
...
public class Inheritance01 {
	...
	public static void main(String[] args) {
		...
		//	, 부모 클래스의 멤버들을 사용 가능
		stv.channel = 10; // 부모 클래스로부터 상속받음 멤버(변수)
