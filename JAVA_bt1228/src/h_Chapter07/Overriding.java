...
//! 오버라디잉의 정의
// : 자식 클래스가 부모 클래스에서 정의된 메서들르 자신의 상황에 맞게 재정의
...
// - 반환타입
//	 : 부모 클래스의 메서드 반환타입과 같거나 자동 형변환이 가능한 것이어야 함.
// - @Overrid(어노테이션)
// 	 : 오버라이딩된 메서드임을 명시적으로 표시

//! 오버라딩의 구현 방법
// 1. 부모 클래스의 상속받은 메서들르 하위 클래스에서 다시 정의
// 2. 메서드의 이름, 반환타입, 매개변수가 부모 클래스의 메서드와 동일하게 작성
// 	  : 자식 클래스의 특성에 맞는 구현부를 재정의
// 3. @Override 어노테이션을 메서드 위에 작성

//! 오버라이딩을 사용하는 이유\
// 1. 다형성을 구현
// : 하나의 이름으로 여러기능을 수행가능한 코드를 작성
...
class Dog2 extends Animal2 {
	...
	// : 코드에 메타데이터를 제공하는 방법
	// @Override
	// : 현재 메서드가 부모 클래스의 메서드를 오버라딩한다는 것을 명시
	@Override
	void sound() {
...
public class Overriding {
	...
	public static void main(String[] args) {
		...
		// -  , 메서드 이름은 같아야 하지만 매개변수의 타입, 개수, 순서 중 적어도 하나는 달라야 함
