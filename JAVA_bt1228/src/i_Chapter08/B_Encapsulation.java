...
class Student {
	...
	// : 점수 값을 설정할 때는 0보다 크고 100보다 작은지 확인하는 검사코드도 추가
	// : 유효하지 않은 점수일 경우 '유효하지 않습니다.' 출력
	public void setScore(int score) {
		if(score > 0 && score < 100) {
			this.score = score;
		} else {
			System.out.println("유호하지 않은 점수입니다.");
		}
	}
	
	
	// getter & setter -  캡슐화 되어 있는 데이터에 접근
	/// 메서드 형식을 get 변수, set 변수로 지정하는 것을 권장
	// : 해당 메서드의 구현목적에 대한 설명
	public String getName() {
		// private 선언이 되어있는 name 변수는 반드시 getName 메서드를 통해서만 
		// , 외부 클래스 또는 외부 패키지에서 접근 가능
		return name;
	}
