...
//! 인터페이스의 필요성
// : 서로 다른 클래스에 대해 공통된 동작을 보장할 떄
// 	 - 인터페이스를 구현하는 모든 클래스는 인터페이스의 모든 메서드를 반드시 구현
// : 여러 인터페이스를 동시에 구현 가능
//   - 한 클래스에 대해 여러개의 인터페이스 구현 가능
...
// - 추상 메서드, 구체적인 메서드, 필드, 생성자 포함 가능

//! 인터페이스 선언 방법
// : 클래스와 작성하는 방법이 유사하지만 키워드 class 대신 interface 를 사용
// : interface 는 접근제어자로 public, default 만 사용 가능

// interface 인터페이스명 {
	// interface 가 가지는 필드는 모두 public static final 특성을 가짐
	// : 모든 멤버변수는 public static final 이어야 하면, 생략 가능
 	// (final)타입 상수명 = 값;

	// 2. 메서드
	// : public abstract 이어야하며, 생략 가능
	// void 메서드명();
// }
